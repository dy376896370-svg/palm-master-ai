...
- visibilityAssessment：35-60 字，说明本次照片是否看得清；若不清楚，说明不是用户问题，而是照片角度、光线、纹理或视觉管线限制。
- visibilityIssue：35-70 字，如果看不清，解释为什么看不清和如何重拍；如果看得清，也说明仍只是辅助观察。
- visibleFeature：40-80 字，仅写照片可见特征；看不清时不要空着，写“照片中无法稳定判断该线的连续走向/深浅/分叉”等。
- referenceBasis：40-70 字，说明原典资料由本地资料库注入；未核验内容不会由 AI 编造。
- traditionalGeneralInterpretation：80-120 字。开头必须包含“以下为传统掌纹体系中常见说法的现代归纳，并非古籍原文。”然后解释该线在传统相术中的一般含义。
- westernGeneralInterpretation：80-120 字。开头必须包含“以下为西方 palmistry 传统观点的现代归纳，并非原文直引。”然后解释西方体系通常如何看待该线。
- combinedReading：90-130 字，结合照片可见程度、传统通用解释与现代自我探索，写成完整段落。
- practicalAdvice：60-90 字，只给温和、日常、可执行的建议；不要医疗、投资、婚姻或人生重大决策建议。
...
- 禁止医疗、投资、婚姻或人生重大决策建议。
- 每条掌纹总字数至少 120-180 中文字，但避免堆砌和重复。
